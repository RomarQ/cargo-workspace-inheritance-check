...
mod tests {
    ...
    fn test_fix_promotion_with_target_specific_default_features() {
        // Create workspace where the dep only appears in target-specific sections
            ...
            std::fs::write(
                ...
                format!(
                    "[package]\nname = \"{name}\"\nversion = \"0.1.0\"\nedition = \"2021\"\n\n\
                     ...
                     winapi = {{ version = \"0.3\", default-features = false }}\n"
                ),
            )
            .unwrap();
            std::fs::write(tmp.path().join(format!("crates/{name}/src/lib.rs")), "")
        ...
        // Workspace dep must pick up default-features = false from target-specific sections
        let root = read_toml(&tmp, "Cargo.toml");
        assert!(
            root.contains("default-features = false"),
        ...
        // Members should have default-features stripped
        for name in &["one", "two"] {
            let member = read_toml(&tmp, &format!("crates/{name}/Cargo.toml"));
            assert!(
                !member.contains("default-features"),
